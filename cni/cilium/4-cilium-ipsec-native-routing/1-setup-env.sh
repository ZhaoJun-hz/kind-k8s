...
cat <<EOF | kind create cluster --name=cilium-ipsec-native-routing --image=my.harbor.cn/k8sstudy/kindest/node:v1.31.4 -v=9 --config=-
kind: Cluster
...
kind load docker-image quay.io/cilium/cilium:$cilium_version quay.io/cilium/operator-generic:$cilium_version quay.io/cilium/cilium-envoy:$cilium_envoy_version --name cilium-ipsec-native-routing
...
helm install cilium cilium/cilium --set k8sServiceHost=$controller_node_ip --set k8sServicePort=6443 --version 1.17.3 --namespace kube-system --set image.pullPolicy=IfNotPresent --set debug.enabled=true --set debug.verbose="datapath flow kvstore envoy policy" --set bpf.monitorAggregation=none --set monitor.enabled=true --set ipam.mode=cluster-pool --set cluster.name=cilium-ipsec-native-routing --set routingMode=native --set autoDirectNodeRoutes=true --set ipv4NativeRoutingCIDR="10.0.0.0/8" --set encryption.enabled=true --set encryption.type=ipsec --set encryption.ipsec.interface=eth0
