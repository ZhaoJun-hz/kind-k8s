...
cat <<EOF | kind create cluster --name=cilium-kubeproxy --image=my.harbor.cn/k8sstudy/kindest/node:v1.31.4 -v=9 --config=-
kind: Cluster
...
kind load docker-image quay.io/cilium/cilium:$cilium_version quay.io/cilium/operator-generic:$cilium_version quay.io/cilium/cilium-envoy:$cilium_envoy_version --name cilium-kubeproxy
